...
//递归函数处理数组
function Find_sons($arr,$id,$lev=0){
    static $list = array();
    foreach($arr as $k=>$v) {
        if($v['pid'] == $id) {
            $v['lev'] = $lev;
            $list[] = $v;
            Find_sons($arr,$v['cat_id'],$lev+3);
        }
    }
    return $list;
}
 ...
 * */
 function smtp_mail( $sendto_email, $subject, $body, $extra_hdrs, $user_name){    
        ...
        $mail->Username = "viisou";     // SMTP username  注意：普通邮件认证不需要加 @域名    
        $mail->Password = "viisou19871102"; // SMTP password    
        $mail->From = "viisou@163.com";      // 发件人邮箱    
        $mail->FromName =  "微搜网服务邮箱";  // 发件人
        $mail->CharSet = "UTF-8";   // 这里指定字符集！    
        $mail->Encoding = "base64";    
        $mail->AddAddress($sendto_email,$user_name);  // 收件人邮箱和姓名    
        $mail->AddReplyTo("viisou@163.com","viisou@163.com");    
